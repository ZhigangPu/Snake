...
bool GenerateNextAction(Game& game){
    std::default_random_engine e(time(0));
    std::uniform_int_distribution<unsigned> randomDirect(0, 10);
    vector<Velocity> otherPossibleVelocity = ComputeOtherVelocity(game);
    if(randomDirect(e) < 2) game.veclociy = otherPossibleVelocity[0];
    if(randomDirect(e) > 8) game.veclociy = otherPossibleVelocity[1];
